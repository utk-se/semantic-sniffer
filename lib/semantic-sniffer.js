...
export default {
    ...
    activate(state) {
        ...
        atom.workspace.observeTextEditors(editor => {
            ...
            let lang = this.snifferLangs[editor.getRootScopeDescriptor()];
            if(lang != undefined) {
              this.editorMarkerLayers[editor.id] = sniffviewlayer;
              this.editorSniffers[editor.id] = new SmellDetector(lang);
    ...
    sniff(n_editor) {
        ...
        let results = detector.getRanges(detector.tree.rootNode);
        for (result of results) {
          for (scanResult of results['captures']) {
